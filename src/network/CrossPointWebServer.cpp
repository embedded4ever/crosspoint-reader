...
void CrossPointWebServer::begin() {
  if (running) {
    //LOG("[%lu] [WEB] Web server already running\n", millis());
    return;
  }
  ...
  //LOG("[%lu] [WEB] [MEM] Free heap before begin: %d bytes\n", millis(), ESP.getFreeHeap());
  //LOG("[%lu] [WEB] Network mode: %s\n", millis(), apMode ? "AP" : "STA");

  //LOG("[%lu] [WEB] Creating web server on port %d...\n", millis(), port);
  ...
  //LOG("[%lu] [WEB] [MEM] Free heap after WebServer allocation: %d bytes\n", millis(), ESP.getFreeHeap());

  if (!server) {
    //LOG("[%lu] [WEB] Failed to create WebServer!\n", millis());
    return;
  }

  // Setup routes
  //LOG("[%lu] [WEB] Setting up routes...\n", millis());
  ...
  //LOG("[%lu] [WEB] [MEM] Free heap after route setup: %d bytes\n", millis(), ESP.getFreeHeap());

  server->begin();
  running = true;

  //LOG("[%lu] [WEB] Web server started on port %d\n", millis(), port);
  // Show the correct IP based on network mode
  const String ipAddr = apMode ? WiFi.softAPIP().toString() : WiFi.localIP().toString();
  //LOG("[%lu] [WEB] Access at http://%s/\n", millis(), ipAddr.c_str());
  //LOG("[%lu] [WEB] [MEM] Free heap after server.begin(): %d bytes\n", millis(), ESP.getFreeHeap());
}
...
void CrossPointWebServer::stop() {
  ...
  //LOG("[%lu] [WEB] STOP INITIATED - setting running=false first\n", millis());
  running = false;  // Set this FIRST to prevent handleClient from using server

  //LOG("[%lu] [WEB] [MEM] Free heap before stop: %d bytes\n", millis(), ESP.getFreeHeap());

  // Add delay to allow any in-flight handleClient() calls to complete
  delay(100);
  //LOG("[%lu] [WEB] Waited 100ms for handleClient to finish\n", millis());

  server->stop();
  //LOG("[%lu] [WEB] [MEM] Free heap after server->stop(): %d bytes\n", millis(), ESP.getFreeHeap());

  // Add another delay before deletion to ensure server->stop() completes
  delay(50);
  //LOG("[%lu] [WEB] Waited 50ms before deleting server\n", millis());

  server.reset();
  //LOG("[%lu] [WEB] Web server stopped and deleted\n", millis());
  //LOG("[%lu] [WEB] [MEM] Free heap after delete server: %d bytes\n", millis(), ESP.getFreeHeap());

  // Note: Static upload variables (uploadFileName, uploadPath, uploadError) are declared
  // later in the file and will be cleared when they go out of scope or on next upload
  //LOG("[%lu] [WEB] [MEM] Free heap final: %d bytes\n", millis(), ESP.getFreeHeap());
}
...
void CrossPointWebServer::handleClient() const {
  ...
  if (!server) {
    //LOG("[%lu] [WEB] WARNING: handleClient called with null server!\n", millis());
    return;
  }

  // Print debug every 10 seconds to confirm handleClient is being called
  if (millis() - lastDebugPrint > 10000) {
    //LOG("[%lu] [WEB] handleClient active, server running on port %d\n", millis(), port);
...
void CrossPointWebServer::scanFiles(const char* path, const std::function<void(FileInfo)>& callback) const {
  ...
  if (!root) {
    //LOG("[%lu] [WEB] Failed to open directory: %s\n", millis(), path);
    return;
  }

  if (!root.isDirectory()) {
    //LOG("[%lu] [WEB] Not a directory: %s\n", millis(), path);
    root.close();
    return;
  }

  //LOG("[%lu] [WEB] Scanning files in: %s\n", millis(), path);
...
void CrossPointWebServer::handleFileListData() const {
  ...
  scanFiles(currentPath.c_str(), [this, &output, &doc, seenFirst](const FileInfo& info) mutable {
    ...
    if (written >= outputSize) {
      ...
      //LOG("[%lu] [WEB] Skipping file entry with oversized JSON for name: %s\n", millis(), info.name.c_str());
      return;
    }
  ...
  server->sendContent("");
  //LOG("[%lu] [WEB] Served file listing page for path: %s\n", millis(), currentPath.c_str());
}
...
void CrossPointWebServer::handleUpload() const {
  ...
  if (!running || !server) {
    //LOG("[%lu] [WEB] [UPLOAD] ERROR: handleUpload called but server not running!\n", millis());
    return;
  }
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    //LOG("[%lu] [WEB] [UPLOAD] START: %s to path: %s\n", millis(), uploadFileName.c_str(), uploadPath.c_str());
    //LOG("[%lu] [WEB] [UPLOAD] Free heap: %d bytes\n", millis(), ESP.getFreeHeap());
    ...
    if (SdMan.exists(filePath.c_str())) {
      //LOG("[%lu] [WEB] [UPLOAD] Overwriting existing file: %s\n", millis(), filePath.c_str());
    ...
    if (!SdMan.openFileForWrite("WEB", filePath, uploadFile)) {
      ...
      //LOG("[%lu] [WEB] [UPLOAD] FAILED to create file: %s\n", millis(), filePath.c_str());
      return;
    }

    //LOG("[%lu] [WEB] [UPLOAD] File created successfully: %s\n", millis(), filePath.c_str());
...
void CrossPointWebServer::handleCreateFolder() const {
  ...
  if (SdMan.mkdir(folderPath.c_str())) {
    //LOG("[%lu] [WEB] Folder created successfully: %s\n", millis(), folderPath.c_str());
    server->send(200, "text/plain", "Folder created: " + folderName);
  } else {
    //LOG("[%lu] [WEB] Failed to create folder: %s\n", millis(), folderPath.c_str());
...
void CrossPointWebServer::handleDelete() const {
  ...
  if (itemName.startsWith(".")) {
    //LOG("[%lu] [WEB] Delete rejected - hidden/system item: %s\n", millis(), itemPath.c_str());
  ...
  for (size_t i = 0; i < HIDDEN_ITEMS_COUNT; i++) {
    if (itemName.equals(HIDDEN_ITEMS[i])) {
      //LOG("[%lu] [WEB] Delete rejected - protected item: %s\n", millis(), itemPath.c_str());
  ...
  if (!SdMan.exists(itemPath.c_str())) {
    //LOG("[%lu] [WEB] Delete failed - item not found: %s\n", millis(), itemPath.c_str());
    server->send(404, "text/plain", "Item not found");
    return;
  }

  //LOG("[%lu] [WEB] Attempting to delete %s: %s\n", millis(), itemType.c_str(), itemPath.c_str());
  ...
  if (itemType == "folder") {
    ...
    if (dir && dir.isDirectory()) {
      ...
      if (entry) {
        ...
        entry.close();
        dir.close();
        //LOG("[%lu] [WEB] Delete failed - folder not empty: %s\n", millis(), itemPath.c_str());
  ...
  if (success) {
    //LOG("[%lu] [WEB] Successfully deleted: %s\n", millis(), itemPath.c_str());
    server->send(200, "text/plain", "Deleted successfully");
  } else {
    //LOG("[%lu] [WEB] Failed to delete: %s\n", millis(), itemPath.c_str());
