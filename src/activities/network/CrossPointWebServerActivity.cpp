...
void CrossPointWebServerActivity::onEnter() {
  ...
  //LOG("[%lu] [WEBACT] [MEM] Free heap at onEnter: %d bytes\n", millis(), ESP.getFreeHeap());
  ...
  //LOG("[%lu] [WEBACT] Launching NetworkModeSelectionActivity...\n", millis());
...
void CrossPointWebServerActivity::onExit() {
  ...
  //LOG("[%lu] [WEBACT] [MEM] Free heap at onExit start: %d bytes\n", millis(), ESP.getFreeHeap());
  ...
  if (dnsServer) {
    //LOG("[%lu] [WEBACT] Stopping DNS server...\n", millis());
    dnsServer->stop();
    delete dnsServer;
    dnsServer = nullptr;
  }
  ...
  //LOG("[%lu] [WEBACT] Waiting 500ms for network stack to flush pending packets...\n", millis());
  delay(500);

  // Disconnect WiFi gracefully
  if (isApMode) {
    //LOG("[%lu] [WEBACT] Stopping WiFi AP...\n", millis());
    WiFi.softAPdisconnect(true);
  } else {
    //LOG("[%lu] [WEBACT] Disconnecting WiFi (graceful)...\n", millis());
    WiFi.disconnect(false);  // false = don't erase credentials, send disconnect frame
  }
  delay(100);  // Allow disconnect frame to be sent

  //LOG("[%lu] [WEBACT] Setting WiFi mode OFF...\n", millis());
  WiFi.mode(WIFI_OFF);
  delay(100);  // Allow WiFi hardware to fully power down

  //LOG("[%lu] [WEBACT] [MEM] Free heap after WiFi disconnect: %d bytes\n", millis(), ESP.getFreeHeap());

  // Acquire mutex before deleting task
  //LOG("[%lu] [WEBACT] Acquiring rendering mutex before task deletion...\n", millis());
  xSemaphoreTake(renderingMutex, portMAX_DELAY);

  // Delete the display task
  //LOG("[%lu] [WEBACT] Deleting display task...\n", millis());
  if (displayTaskHandle) {
    vTaskDelete(displayTaskHandle);
    displayTaskHandle = nullptr;
    //LOG("[%lu] [WEBACT] Display task deleted\n", millis());
  }

  // Delete the mutex
  //LOG("[%lu] [WEBACT] Deleting mutex...\n", millis());
  vSemaphoreDelete(renderingMutex);
  renderingMutex = nullptr;
  //LOG("[%lu] [WEBACT] Mutex deleted\n", millis());

  //LOG("[%lu] [WEBACT] [MEM] Free heap at onExit end: %d bytes\n", millis(), ESP.getFreeHeap());
}
...
void CrossPointWebServerActivity::onNetworkModeSelected(const NetworkMode mode) {
  ...
  if (mode == NetworkMode::JOIN_NETWORK) {
    ...
    //LOG("[%lu] [WEBACT] Turning on WiFi (STA mode)...\n", millis());
    WiFi.mode(WIFI_STA);

    state = WebServerActivityState::WIFI_SELECTION;
    //LOG("[%lu] [WEBACT] Launching WifiSelectionActivity...\n", millis());
...
void CrossPointWebServerActivity::onWifiSelectionComplete(const bool connected) {
  //LOG("[%lu] [WEBACT] WifiSelectionActivity completed, connected=%d\n", millis(), connected);
  ...
  if (connected) {
    ...
    if (MDNS.begin(AP_HOSTNAME)) {
      //LOG("[%lu] [WEBACT] mDNS started: http://%s.local/\n", millis(), AP_HOSTNAME);
    }
...
void CrossPointWebServerActivity::startAccessPoint() {
  //LOG("[%lu] [WEBACT] Starting Access Point mode...\n", millis());
  //LOG("[%lu] [WEBACT] [MEM] Free heap before AP start: %d bytes\n", millis(), ESP.getFreeHeap());
  ...
  if (!apStarted) {
    //LOG("[%lu] [WEBACT] ERROR: Failed to start Access Point!\n", millis());
    onGoBack();
    return;
  }
  ...
  //LOG("[%lu] [WEBACT] Access Point started!\n", millis());
  //LOG("[%lu] [WEBACT] SSID: %s\n", millis(), AP_SSID);
  ...
  if (MDNS.begin(AP_HOSTNAME)) {
    ...
    //LOG("[%lu] [WEBACT] WARNING: mDNS failed to start\n", millis());
  }
  ...
  //LOG("[%lu] [WEBACT] DNS server started for captive portal\n", millis());

  //LOG("[%lu] [WEBACT] [MEM] Free heap after AP start: %d bytes\n", millis(), ESP.getFreeHeap());
...
void CrossPointWebServerActivity::startWebServer() {
  //LOG("[%lu] [WEBACT] Starting web server...\n", millis());
  ...
  if (webServer->isRunning()) {
    ...
    //LOG("[%lu] [WEBACT] Web server started successfully\n", millis());
    ...
    //LOG("[%lu] [WEBACT] Rendered File Transfer screen\n", millis());
  } else {
    //LOG("[%lu] [WEBACT] ERROR: Failed to start web server!\n", millis());
    webServer.reset();
    // Go back on error
    onGoBack();
  }
...
void CrossPointWebServerActivity::stopWebServer() {
  if (webServer && webServer->isRunning()) {
    //LOG("[%lu] [WEBACT] Stopping web server...\n", millis());
    webServer->stop();
    //LOG("[%lu] [WEBACT] Web server stopped\n", millis());
  }
  webServer.reset();
}
